...
#include <fstream>
using namespace std;
int obn,ext,prnt_once=0,dir_Student=0;
...
void laboratry_booking::first()
{
    ...
    system("clear");
    cout<<"\n\n\t\t\t\t\t\t\tDEAR STUDENT TO BOOK SEATS PRESS 'y' or 'Y' \n";
    cout<<"\t\t\t\t\t\t\t";
    cin>>cus;
    //HERE I HAVE TO ADD THE Student LOGIN FUNCTION
     if((cus=='y')||(cus=='Y'))//the Student part starts here
     
     {  cout<<"\n\t\t\t\t\t\t\tIF YOU ARE NEW STUDENT PRESS Y/y TO SIGNUP\n";
        cout<<"\t\t\t\t\t\t\t";
        cin>>cus_in;
        if(cus_in=='Y'||cus_in=='y')
        (*this).data_str();
        else
        {
            cout<<"\n\t\t\t\t\t\t\tWELCOME EXISTING STUDENT!!!\n";
...
fin.open("laboratry_test2.csv");
   ...
   getline(fin,name[i],',');
    fin.close();
             cout<<"\n\n\t\t\t\t\t\t\tCHOOSE THE NUMBER CORRENSPONDING TO TIMING TO BOOK TICKET FOR LABORATRY :-"<<endl;
...
void laboratry_booking::show_seat_frm_file()
{int i=0,j;
...
while(i<m)
{
    ...
    cout<<"\t\t\t\t\t\t\t";
    cout<<"THE TIMINGS OF THE LABORATRY : "<<name[i]<<" :-  "<<endl;
    int j=0;
...
void laboratry_booking::show()
{
    int i,j;
    p=(int*)(malloc(sizeof(int)));
    cout<<"\n\n\t\t\t\t\t\t\tENTER THE NUMBER STUDENT RUNNING IN LABORATRY\n";
    cout<<"\t\t\t\t\t\t\t";
    cin>>m;
    (*p)=m;
     ofstream fout;
    fout.open("laboratry_test2.csv",ios::app);
for(i=0;i<m;i++)//m is the total no of movies running in that hall
{   fflush(stdin);
    cout<<"\n\n\t\t\t\t\t\t\t Name of Sheat "<<i+1<<endl;
    cout<<"\t\t\t\t\t\t\t";
    getline(cin,name[i]);
    fflush(stdin);
    fout<<name[i]<<",";//movie name writing in file
   // cout<<"\n\n\t\t\t\t\t\t\tENTER THE NUMBER : "<<name[i]<<" IN A DAY : ";
    cin>>nt;
    str_tim[i]=nt;
    
    cout<<"\n\n\t\t\t\t\t\t\tENTER THE SHOW TIMINGS OF THE LABORATRY : "<<name[i]<<endl;
    for(j=0;j<nt;j++)
    {   fflush(stdin);
         cout<<"\t\t\t\t\t\t\t";
        getline(cin,tim[i][j]);
...
void laboratry_booking::seatdisp(int z,int wt)
//1st argument to display the seats of that particular laboratry
//2nd arg is for the particular laboratry timeing
{
    arg=z;
    sti=wt;
    int i,j;
...
fin.open("laboratry_test2.csv");
   ...
   getline(fin,name[i],',');
   cout<<"\n\n\t\t\t\t\t\t\tTHIS IS THE SEAT MATRIX FOR LABORATRY: "<<name[i]<<"\n\n";
    fin.close();
    for(i=0;i<13;i++)
    {
        ch='A';
...
void laboratry_booking::book(int z,int wt ,int a)//first argument is for the laboratry number and 3rd argument for manager mode
...
void laboratry_booking::seatin(int z,int wt)//1st argument for the particular laboratry seat booking
//2nd arg is for particular show time of particular laboratry
{
...
fin.open("laboratry_test2.csv");
   ...
   getline(fin,name[i],',');
            ...
            cout<<"\t\t\t\t\t\t\t laboratry_booking="<<name[i]<<endl;
           fin.close();
            cout<<"\t\t\t\t\t\t\t ";
    cin>>n;
    tkt_pr=n;
...
void laboratry_booking::manager()
{   char mov='c',in='c';
    (*this).show();
    while((mov=='c')||(mov=='C'))//this loop is for entering the seats that are booked 
    {
    ...
    cout<<"\n\n\t\t\t\t\t\t\tPRESS 'C' or 'c' IF U WANT TO CONTINUE ENTERING FOR OTHER LABORATRY SHEAT \n";
    cout<<"\t\t\t\t\t\t\t";
    cin>>mov;
    in='c';
    }
...
void laboratry_booking::ticket_print()
{    int i,a=0,b=0,d=0;
    char c;
    string time;//time to store the laboratry_booking timing
...
cout<<"\t\t\t|                                                                       |\n";
cout<<"\t\t\t|                  THE DETAILS:                                         |\n";
cout<<"\t\t\t|                            LABORATRY HALL: "<<hall_name<<"                 \n";
cout<<"\t\t\t|                            LABORATRY NAME: "<<name[i]<<"                                 \n";
...
void laboratry_booking::pass_proctect()//to protect the password
{
...
void laboratry_booking::data_str()
{
 ofstream fout;
...
int laboratry_booking::password_check()
{
     cout<<"\n\n\t\t\t\t\t\t\tENTER YOUR USER ID REG.NO.\n";
            fflush(stdin);
            cout<<"\t\t\t\t\t\t\t";
...
{
    prnt_once=0;
    laboratry_booking obj[4];
    while(1)
    {
        cout<<"\n\n\t\t\t\t\t\t\t PRESS 1 TO BOOK THE LABORATRY SHEATS";
